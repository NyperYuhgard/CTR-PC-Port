...
void Platform_PollHostEvents(void)
{
        ...
        {
                /* Forward every event to the netplay chat window handler.
                 * If the event belongs to the chat window, it's consumed
                 * and we skip the normal game processing (which would
                 * otherwise close the game on WINDOW_CLOSE_REQUESTED
                 * or treat chat keypresses as game input). */
#ifdef CTR_NATIVE
                if (Netplay_HandleSDLEvent(&event))
                        continue;
#endif
