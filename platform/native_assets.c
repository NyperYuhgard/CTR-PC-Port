...
#define NATIVE_ASSETS_BIGFILE_DIR                "BIGFILE"
...
internal int NativeAssets_DirExists(const char *path)
{
        struct stat st;
...
internal int NativeAssets_BaseHasRequiredFile(NativeStr8 baseDir)
{
        ...
        BFDBG_PRINTF("Assets: BaseHasRequiredFile — checking base: %s", baseDir.ptr ? baseDir.ptr : "(null)");
        ...
        {
                BFDBG_PRINTF("Assets: checking BIGFILE.BIG at: %s", path);
                if (NativeAssets_FileExists(path))
                {
                        BFDBG_PRINTF("Assets: BIGFILE.BIG FOUND at: %s", path);
                        return 1;
                }
                BFDBG_PRINTF("Assets: BIGFILE.BIG not found at: %s", path);
        }

        /* Also accept unpacked BIGFILE/ folder as a valid asset source */
        if (NativePath_Join(path, sizeof(path), baseDir, NATIVE_STR8_LIT(NATIVE_ASSETS_DIR_NAME "/" NATIVE_ASSETS_BIGFILE_DIR)))
        {
                BFDBG_PRINTF("Assets: checking BIGFILE/ folder at: %s", path);
                if (NativeAssets_DirExists(path))
                {
                        BFDBG_PRINTF("Assets: BIGFILE/ folder FOUND at: %s", path);
                        return 1;
                }
                BFDBG_PRINTF("Assets: BIGFILE/ folder not found at: %s", path);
        }

        BFDBG_PRINTF_ERR("Assets: NEITHER BIGFILE.BIG nor BIGFILE/ found under base: %s", baseDir.ptr ? baseDir.ptr : "(null)");
        return 0;
}
...
int NativeAssets_Init(const char *executableBasePath)
{
        ...
        BFDBG_PRINTF("Assets: Init — exeBasePath=%s", executableBasePath ? executableBasePath : "(null)");
        ...
        {
                BFDBG_PRINTF("Assets: Init — found assets in exe dir");
        ...
        if (NativePath_Parent(parentDir, sizeof(parentDir), exeDir))
        {
                ...
                BFDBG_PRINTF("Assets: Init — trying parent dir: %s", parentDir);

                if (NativeAssets_BaseHasRequiredFile(parent))
                {
                        BFDBG_PRINTF("Assets: Init — found assets in parent dir");
                        return NativeAssets_SetBaseDir(parent);
                }
        }

        BFDBG_PRINTF_ERR("Assets: Init — no assets found, falling back to exe dir");
        return NativeAssets_SetBaseDir(exeDir);
}
...
{
        fprintf(stderr, "[CTR Native] Required: %s (or unpacked %s/ folder), %s, %s, %s, plus XA files referenced by %s\n",
                NATIVE_ASSETS_BIGFILE_PATH, NATIVE_ASSETS_BIGFILE_DIR,
...
internal int NativeAssets_CheckRequiredFileOrDir(const char *path, const char *dirPath)
{
        char assetPath[NATIVE_ASSETS_PATH_MAX];
        ...
        /* Check for the directory alternative */
        if (NativeAssets_BuildPath(dirPath, assetPath, sizeof(assetPath)))
        {
                if (NativeAssets_DirExists(assetPath))
                        return 1;
        }

        fprintf(stderr, "[CTR Native] missing asset: %s (or unpacked %s/ folder)\n", path, dirPath);
        return 0;
}
...
int NativeAssets_Validate(void)
{
        ...
        BFDBG_PRINTF("Validate: checking required assets...");

        /* Accept either BIGFILE.BIG or unpacked BIGFILE/ folder */
        ok &= NativeAssets_CheckRequiredFileOrDir(NATIVE_ASSETS_BIGFILE_PATH, NATIVE_ASSETS_BIGFILE_DIR);
        ...
        if (ok)
        {
                BFDBG_PRINTF("Validate: basic assets OK, checking XA files...");
                ok &= NativeAssets_ValidateXA();
        }

        if (!ok)
        {
                BFDBG_PRINTF_ERR("Validate: FAILED — some assets are missing!");
                NativeAssets_PrintHeader();
                NativeAssets_PrintFooter();
        }
        else
        {
                BFDBG_PRINTF("Validate: all assets OK");
        }
