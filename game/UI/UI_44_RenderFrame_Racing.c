...
void UI_RenderFrame_Racing()
{
        ...
        if (playerThread != 0)
        {
                ...
                do
                {
                        ...
                        // We check g_NetplayRacing alone (not "numPlyr < 2")
                        // because in netplay numPlyrCurrGame is 2 even though
                        // each instance is single-player locally — otherwise
                        // the HUD would render twice (once per driver).
                        if (g_NetplayRacing)
                        {
                        ...
                        if (
                            // numPlyrCurrGame is less than 2 (1P mode) OR netplay
                            (numPlyr < 2
#ifdef CTR_NATIVE
                             || g_NetplayRacing
#endif
