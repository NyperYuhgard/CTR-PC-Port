...

{
        ...
        /* Netplay: always use the 1P HUD layout (index 0) even though
         * numPlyrCurrGame is 2. Each netplay instance renders in full-screen
         * 1P mode, so the 2P split-screen layout (index 1) would place HUD
         * elements in the wrong positions (e.g. rank number off-screen,
         * wumpa 3D model misplaced). */
#ifdef CTR_NATIVE
        {
                int hudIndex = g_NetplayRacing ? 0 : (gGT->numPlyrCurrGame - 1);
                hudStruct = data.hudStructPtr[hudIndex];
        }
#else
...
#ifdef CTR_NATIVE
                /* Netplay: only create HUD 3D instances (big number, fruit
                 * dispenser, etc.) for the LOCAL driver. Without this, the
                 * HUD elements get created for every driver (host + client)
                 * and end up rendered twice on screen — once per driver.
                 * Each instance is single-player locally even though
                 * numPlyrCurrGame is 2 in netplay. */
