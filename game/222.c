...
#ifdef CTR_NATIVE
        /* Netplay: remote drivers don't have instBigNum / instFruitDisp
         * because UI_INSTANCE_BirthWithThread only creates HUD 3D
         * instances for the local driver. Skip the scale check to avoid
         * NULL dereference. */
        if (g_NetplayRacing && driver->driverID != Netplay_GetLocalPlayerId())
        {
                /* Remote driver — no HUD 3D instances, skip */
        }
        else
#endif
        if (driver->instBigNum != NULL &&
            driver->instBigNum->scale[0] != AA_BIG_NUM_TARGET_SCALE)
        {
                struct Instance *instFruitDisp = driver->instFruitDisp;
                if (instFruitDisp != NULL)
...
#ifdef CTR_NATIVE
                /* Netplay: remote drivers don't have instBigNum/instFruitDisp.
                 * Skip them — only the local player has HUD 3D instances. */
                if (g_NetplayRacing && i != Netplay_GetLocalPlayerId())
                        continue;
#endif
...
void AA_EndEvent_DisplayTime(s16 driverId, s16 timeOffsetFrames)
{
        ...
        struct Instance *bigNum = driver ? driver->instBigNum : NULL;

        /* If the driver or its bigNum instance doesn't exist (e.g. remote
         * driver in netplay), skip the 3D number rendering. */
        if (driver == NULL || bigNum == NULL)
                return;
