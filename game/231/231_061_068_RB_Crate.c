...
void RB_CrateAny_ExplodeInit(struct Instance *crateInst, int color)
{
        ...
        if (g_NetplayRacing && crateInst->model != NULL)
        {
                ...
                /* Compute a stable crateID so the remote machine can match this
                 * crate even if its local position has shifted slightly. We
                 * scan level1->ptrInstDefs to find our index. */
                hit.crateID = 0;
                if (sdata->gGT != NULL && sdata->gGT->level1 != NULL && sdata->gGT->level1->ptrInstDefs != NULL)
                {
                        int num = sdata->gGT->level1->numInstances;
                        int i;
                        for (i = 0; i < num; i++)
                        {
                                if (sdata->gGT->level1->ptrInstDefs[i].ptrInstance == crateInst)
                                {
                                        hit.crateID = Netplay_ComputeCrateID((int)sdata->gGT->levelID, i);
                                        break;
                                }
                        }
                }
                Netplay_QueueCrateHit(&hit);
        }
#endif
