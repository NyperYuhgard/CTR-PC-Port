...
void MainInit_VisMem(struct GameTracker *gGT)
{
        ...
        /* VisMem: must be initialized for ALL drivers (including remote
         * peers), not just the local viewport. Using NUM_LOCAL_PLAYERS here
         * would leave visMem->visLeafSrc[1+] as garbage; if the engine
         * processes a remote driver's visibility it would crash. */
