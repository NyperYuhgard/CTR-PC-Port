...
void MainFrame_RenderFrame(struct GameTracker *gGT, struct GamepadSystem *gGamepads)
{
        ...
        if ((gGT->renderFlags & 8) != 0 && gGT->stars.numStars != 0)
                RenderStars(&gGT->pushBuffer[0], &gGT->backBuffer->primMem, &gGT->stars, NUM_LOCAL_PLAYERS(gGT));

        if (((gGT->renderFlags & 0x100) != 0) && (NUM_LOCAL_PLAYERS(gGT) > 1))
                DecalMP_01(gGT);
        ...
        if (((gGT->renderFlags & 1) != 0) && (ptr_mesh_info != 0))
        {
                ...
                if (((gGT->hudFlags & 1) != 0) && (NUM_LOCAL_PLAYERS(gGT) > 1))
...
void RainLogic(struct GameTracker *gGT)
{
        ...
        numPlyrCurrGame = NUM_LOCAL_PLAYERS(gGT);
...
{
        int numPlyrCurrGame = NUM_LOCAL_PLAYERS(gGT);
...
void RenderAllHUD(struct GameTracker *gGT)
{
        ...
        if ((NUM_LOCAL_PLAYERS(gGT) == 1) && ((hudFlags & 8) != 0) && ((gameMode1 & START_OF_RACE) != 0))
        {
...
void RenderAllNormalParticles(struct GameTracker *gGT)
{
        ...
        for (i = 0; i < NUM_LOCAL_PLAYERS(gGT); i++)
        {
...
void RenderAllHeatParticles(struct GameTracker *gGT)
{
        ...
        Torch_Main(gGT->particleList_heatWarp, &gGT->pushBuffer[0], &gGT->backBuffer->primMem, NUM_LOCAL_PLAYERS(gGT), gGT->swapchainIndex * 0x128);
}
