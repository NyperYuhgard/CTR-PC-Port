...
void LOAD_NextQueuedFile()
{
        ...
        if ((sdata->queueReady != 0) && (sdata->XA_State == 0) && (sdata->queueLength != 0))
        {
                ...
                BFDBG_PRINTF("NextQueue: dequeued type=%d subfile=%d callback=%p",
                        curr->type_UNUSED, curr->subfileIndex, (void *)curr->callbackFuncPtr);
        ...
        if (sdata->frameFinishedVRAM != 0)
        {
                ...
                {
                        BFDBG_PRINTF("NextQueue: VRAM 3-frame wait done, invoking user callback %p",
                                (void *)curr->callbackFuncPtr);
                        ...
                        sdata->queueReady = 1;
                        BFDBG_PRINTF("NextQueue: VRAM path complete, queueReady=1");
                }
        }
}
