...
// If this is human and not AI
        ...
        /* In netplay, use the real numPlyrCurrGame (number of connected peers)
         * because the race-end logic needs to know how many humans are playing.
         * Using NUM_LOCAL_PLAYERS (=1 in netplay) would end the race as soon
         * as the local player finishes, ignoring other peers still racing.
         * The host signals return-to-lobby separately via RETURN_LOBBY. */
#ifdef CTR_NATIVE
        int numPlyr = g_NetplayRacing ? gGT->numPlyrCurrGame : NUM_LOCAL_PLAYERS(gGT);
#else
